...
from utils.db import select_query_dict, insert_row
...
def createroom():
    ...
    #     'createroom.html', roomId=user.roomId, roomCode=user.roomCode
    # )
        ...
        try:
            ...
            if room.validate_room():
                ...
                if not room.add_user(current_user.userId):
                    flash(
                        'you are already in the game or not allowed to join.'
                    )
                    return redirect(
                        url_for('ui_blueprint.room', roomId=user.roomId)
                    )
                current_app.logger.info("user has been added to room.")
                flash(f'Joined room {form.roomId.data}')
                return redirect(
                    url_for('ui_blueprint.room', roomId=room.roomId)
                )
            else:
...
def room(roomId):
    ...
    if current_user.userId not in room.players:
        flash(
    ...
    if gameStarted:
        infoLogger('game is already started. redirecting')
        return redirect(url_for('ui_blueprint.play', roomId=room.roomId))
    flash(
        'current room is'
        f': {room.roomId}'
...
def play(roomId):
    ...
    if request.method == "POST":
        ...
        if x.get("gameType") == "pass":
            if room.gameSelectorAlt:
                ...
                return redirect(
                    url_for('ui_blueprint.play', roomId=room.roomId)
                )
            else:
                room.drop_game()
                return redirect(
        ...
        if room.gameSelectorAlt:
            ...
            room.update_gameSelector(
                room.get_previousPlayer(current_user.userId)
            )
    ...
    if room.check_ifCardsFinished():
        return redirect(
            url_for('ui_blueprint.game_statistics', roomId=room.roomId)
        )
    ...
    if not card.suitMapper.get(room.gameType):
        ...
        out["openCards"] = Card(
            room.cards.get(out["openCardPlayer"])
        ).map_cardToDictDeck()
    ...
    for player, acard in room.get_currentBufferCards().items():
        currentBufferCards[
            room.playersMapping[player]
        ] = card.convert_aCardToVisual(acard)
    # pointsTable = room.get_pointsTable()
